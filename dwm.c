...
static void movelayout(const Arg *arg);
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext) - lrpad; /* remove +2 (what it mean 2px right padding) */
		while (1) {
			if ((unsigned int)*ts > LENGTH(colors)) { ts++; continue ; }
			ctmp = *ts;
			*ts = '\0';
			drw_text(drw, m->ww - tw + tx, 0, tw - tx, bh, 0, tp, 0);
			tx += TEXTW(tp) - lrpad + PAD_COLOUR;
			if (ctmp == '\0') { break; }
...
tagmon(const Arg *arg, const int moveview)
{
    ...
    // FIXME: check why is not working
    if (moveview)
        focusmon(arg);
}
...
moveview(const Arg *arg)
{
    ...
    if (arg->ui) { 
        ...
        if (newtag == 0)
            newtag = selmon->tagset[selmon->seltags];
    } else {
        newtag = selmon->tagset[selmon->seltags] << 1;
